...
export default function AmolManagementPage() {
  ...
  const { data: apiResponse, isLoading } = useQuery({
    queryKey: ["amol-resources", selectedCategory, searchQuery, currentPage],
    queryFn: async () => {
      let url = `/library/resources?section=amol&page=${currentPage}&limit=8`;
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire(
        "আলহামদুলিল্লাহ্‌!",
        editId
          ? "আমলের তথ্য আপডেট সম্পন্ন হয়েছে।"
          : "নতুন আমল/দু'আ সফলভাবে যোগ করা হয়েছে।",
        "success",
      );
      queryClient.invalidateQueries({ queryKey: ["amol-resources"] });
      resetForm();
    },
    onError: (err: any) => {
      Swal.fire(
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire(
        "রিমুভড!",
        "আমল রেকর্ডটি স্থায়ীভাবে মুছে ফেলা হয়েছে।",
        "success",
      );
      queryClient.invalidateQueries({ queryKey: ["amol-resources"] });
    },
  ...
  const handleDelete = (id: string) => {
    Swal.fire({
      title: "নিশ্চিত তো?",
      text: "এই আমল বা দু'আটি লাইব্রেরি থেকে স্থায়ীভাবে মুছে ফেলা হবে!",
      icon: "warning",
  ...
  const resetForm = () => {
    setFormData({ title: "", category: "duwa", subType: "", textContent: "" });
    setEditId(null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (
      ...
      return Swal.fire(
        "정보 প্রয়োজন",
        "দয়া করে শিরোনাম, শ্রেণী এবং বিস্তারিত কন্টেন্ট লিখুন।",
        "warning",
      );
    }
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="bg-[#0B3D2E] rounded-3xl p-6 md:p-8 text-white shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-black tracking-tight flex items-center gap-2">
              আমল কন্ট্রোল প্যানেল
            </h1>
            <p className="mt-1.5 text-xs md:text-sm text-slate-300 font-medium">
              নামাজ, প্রতিদিনের দু'আ, জিকির সংক্রান্ত কন্টেন্ট ও টেক্সটসমূহ
              ম্যানেজ করুন।
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="md:col-span-2 flex flex-wrap gap-2">
            {[
              { label: "সকল আমল", value: "all" },
              { label: "নামাজ", value: "namaz" },
              { label: "দু'আ ও জিকির", value: "duwa" },
              { label: "তারাবীহ ও রমাদান", value: "tarabihi" },
            ].map((cat) => (
              <button
                key={cat.value}
                onClick={() => {
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="আমল বা দু'আর নামে সার্চ করুন..."
              value={searchQuery}
              onChange={(e) => {
        ...
        {/* ডাইনামিক ৪-কলাম রেসপন্সিভ কার্ড গ্রিড লেআউট */}
        {isLoading ? (
          ...
          <div className="bg-white border border-neutral-200 text-center py-16 rounded-[2rem] text-sm font-black text-slate-900">
            কোনো আমল বা দু'আ খুঁজে পাওয়া যায়নি।
          </div>
        )}
              ...
              >
                <div className="bg-[#0B3D2E] text-white p-5 flex items-center justify-between">
                  <h3 className="text-base font-black flex items-center gap-2">
                    {editId
                      ? "আমল/দু'আর তথ্য এডিট করুন"
                      : "নতুন আমল কন্টেন্ট ফর্ম"}
                  </h3>
                  <button
                    onClick={resetForm}
                ...
                >
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="উদা: আয়াতুল কুরসি / সকালের জিকির"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      >
                        <option value="duwa">দু'আ ও জিকির</option>
                        <option value="namaz">নামাজ শিক্ষা</option>
                        <option value="tarabihi">তারাবীহ ও রমাদান</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-xs font-black text-slate-900 mb-1.5 uppercase">
                      ...
                      <input
                        type="text"
                        placeholder="উদা: হিসনুল মুসলিম / সালাতের মাসআলা"
        ...
        {/* সম্পূর্ণ কন্টেন্ট রিড মোডাল পপআপ */}
        <AnimatePresence>
          {viewingContent && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
              <motion.div
